...
function crearInterfaz() {
    ...
    label.innerText = "¿Cómo te llamas, héroe?";
    //Lo añadimos al body
    ...
    //Añadimos al body el contenedor
...
function iniciarJuego() {
    ...
    // Habilitar el botón "Jugar" solo si el nombre es válido
...
function tirarDado() {
    ...
    //Igualamos la variable globar numeroResultado a un número aleatorio generado con Math.random (del 1 al 6)
...
function moverHeroe(nuevaFila, nuevaColumna) {
    ...
    //VICTORIA
    //Validación de la victoria en el caso de llegar a la esquina inferior derecha (celda 9, 9)
    if (posicionHeroe.x === 9 && posicionHeroe.y === 9) {
        //Incluimos un retraso de 100 milisegundos con una función anónima para que se vea el desplazamiento del personaje antes de que aparezca el alert
        setTimeout(function () {
            alert("¡HAS GANADO! Encontraste el globo en " + numeroTiradas + " tiradas.");
            ...
            if (recordPuntos === null || numeroTiradas < parseInt(recordPuntos)) {
                alert("¡NUEVO RÉCORD! Has supeado el récord anterior.");
                ...
                // Actualizamos el texto en pantalla con el nuevo récord
                let infoRecord = document.getElementById("infoRecord");
                if (infoRecord) {
                    infoRecord.innerText = "Récord actual: " + nombreJugador + " - " + numeroTiradas + " tiradas";
                }

            } else {
                alert("No has superado el récord de " + localStorage.getItem("recordNombre") + " (" + recordPuntos + " tiradas). ¡Inténtalo de nuevo!");
            }
